...
BACKSPACE:
      ...
      JSR ECHO
NEXTCHAR:
      JSR ACIAin           ; ? get character from console/keyboard
;         LDA ACIAStatus      ;*See if we got an incoming char
;         AND #$01            ;*Test bit 3
;         CMP #$01
;         BNE NEXTCHAR        ;*Wait for character
;         LDA ACIAData        ;*Load char
      BCC NEXTCHAR         ; ?ACIAin function sets carry IF data is present, so if caary clear, wait for character
...
GETCHAR:
      ...
      AND #$01        ;Test bit 3
      CMP #$01
      BNE GETCHAR     ;Wait for character
      LDA ACIAData    ;Load char
      RTS

MSG1:
      .byte "Welcome to EWOZ 1.0.",0
MSG2:
      .byte "Start Intel Hex code Transfer.",0
MSG3: 
      .byte "Intel Hex Imported OK.",0
MSG4:
      .byte "Intel Hex Imported with checksum error.",0
